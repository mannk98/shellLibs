...
pushed). Gates: `make test` → **102/102 green**, `make lint-ci` → **exit 0**. Dev machine is
...
- **Structure tidy-up — Theme C** (commit f01615c): `database-utils` eval→argv via a single `_mysql_exec` helper — removed 13 `eval` sites + the shell-injection class, `_confirm` on drop/truncate, fixed the arity / `mysql_native_password` / `psql -W` bugs, kept the REPLs, added `tests/database-utils.bats`. **Remaining audit themes A (local-sweep) / E (more merges) / F (renames) / G (def-style) not started — see the audit doc.**
...
- [ ] **Theme A — `local` sweep** (~90 sites; database-utils already done with C): skip `MANNK_MYSQL_*`/`GOPRIVATE` exports + lazy-ensurer cached globals (`$oscheck`/`$nginxgenConName`/`_LOGSHELL_HAS_NANOS`).
