...
pushed). Gates: `make test` → **108/108 green**, `make lint-ci` → **exit 0**. Dev machine is
...
- **Structure tidy-up — Theme A** (commits 7d5bac0 + 9ea48b0): `local` sweep — ~90 leaking positional assignments declared `local` across 9 files so they stop persisting as shell globals; command-subs split for SC2155; intentional exports + lazy-ensurer caches skipped; no-leak regression tests added. **Remaining audit themes E (more merges) / F (renames) / G (def-style) not started — see the audit doc.**
